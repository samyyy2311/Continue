...
use bytes::BytesMut;
use limits::MAX_FRAME_NOTIFICATION_BYTES;
...
pub async fn write_msg<M: prost::Message>(
    ...
    let frame = protocol::encode_frame(msg, MAX_FRAME_NOTIFICATION_BYTES)?;
    stream
        .write_all(&frame)
        .await
...
) -> Result<M, NotificationError> {
    let mut buf = BytesMut::with_capacity(1024);
    let mut chunk = [0u8; 1024];
    loop {
        if let Some(msg) = protocol::decode_frame_from_buf::<M>(&mut buf, MAX_FRAME_NOTIFICATION_BYTES)? {
            return Ok(msg);
        }
